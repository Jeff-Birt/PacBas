...
255 rem PacBas02-5.bas simplify the Pac move section, add score print (239 lt)
260 rem ------------------------------------------------------------------------
...
370 rem ****** Move player ************
380 for i=. to 1000
390 j=peek(j2):poke py+px,sc:rem read joystick, clear Pac position
400 rem ** can move north
410 if (j and js)=. then t=py-yo:if peek(t+px)<>wa then py=t:goto 550
440 rem ** can move south
450 if (j and jn)=. then t=py+yo:if peek(t+px)<>wa then py=t:goto 550
480 rem ** can move east
490 if (j and jw)=. then t=px+w1:if peek(py+t)<>wa then px=t:goto 550
520 rem ** can move west
530 if (j and je)=. then t=px-w1:if peek(py+t)<>wa then px=t
550 pt=abs(pt-w1):poke py+px,pc(pt):rem poke new position and toggle char
560 rem -------------------------------
...
690 poke gy+gx,gh
692 rem go to top left corner of screen and print score
694 print "{home}Score 1234""
700 next i
710 print ti$
720 end
