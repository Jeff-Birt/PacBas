...
320 rem ****** Move player ************
325 for i=. to 1000
330 j=peek(j2):poke p(y)+p(x),sc:rem read joystick, clear current pac position
340 rem ** can move north, dont check if joystick south
350 if (j and js) then goto 390
360 t=p(y)-yo:if peek(t+p(x))<>wa then p(y)=t
370 goto 490
380 rem ** can move south, dont check if joystick north
390 if (j and jn) then goto 430
400 t=p(y)+yo:if peek(t+p(x))<>wa then p(y)=t
410 goto 490
420 rem ** can move east, dont check if joystick west
430 if (j and jw) then goto 470
440 t=p(x)+1:if peek(p(y)+t)<>wa then p(x)=t
450 goto 490
460 rem ** can move west, dont check if joystick east
470 if (j and je) then goto 490
480 t=p(x)-1:if peek(p(y)+t)<>wa then p(x)=t
490 pt=abs(pt-1):poke p(y)+p(x),pc(pt)
495 next i
496 print ti$
498 end
500 rem gosub 540
...
1110 DIM p(1):DIM g1(2):DIM g2(2):DIM pc(1)
1120 x=0:y=1:l=2:yo=40:t=0:w4=4:w2=2:w1=1
1125 jn=2:js=1:je=4:jw=8: rem joystick bits
1130 n=.:s=.:e=.:w=.:rem direction bias
1140 xt=.:yt=.:lt=.
1150 p(x)=19:p(y)=17*yo+1024
