...
1030 gh(x,0)=3:gh(y,0)=5*yoff+1024:gh(l,0)=3:g=0
...
1290 g=0:rem xtg=gh(x,0):ytg=gh(y,0):ltg=gh(l,0)
1330 gosub 1515
1340 rem gh(x,0)=xtg:gh(y,0)=ytg:gh(l,0)=ltg
1380 rem ******* Move Ghost 2 **********
1390 rem xtg=gh(x,1):ytg=gh(y,1):ltg=gh(l,1)
1400 g=1:gosub 1515
1410 rem gh(x,1)=xtg:gh(y,1)=ytg:gh(l,1)=ltg
1480 return
1490 rem -----------------------------------------------------------------------
...
1515 poke gh(y,g)+gh(x,g),32:n=0:s=0:e=0:w=0:rem clear old position
1518 rem ** can move north
1520 if peek(gh(y,g)-yoff+gh(x,g))=wall then goto 1535
1525 n=n+w1:if gh(y,g)>pa(1) then n=n+w4
1530 rem ** can move south
1535 if peek(gh(y,g)+yoff+gh(x,g))=wall then goto 1550
1540 s=s+w1:if gh(y,g)<pa(1) then s=s+w4
1545 rem ** can move east
1550 if peek(gh(y,g)+gh(x,g)+1)=wall then goto 1565
1555 e=e+w1:if gh(x,g)<pa(0) then e=e+w4
1560 rem ** can move west
1565 if peek(gh(y,g)+gh(x,g)-1)=wall then goto 1578
1570 w=w+w1:if gh(x,g)>pa(0) then w=w+w4
1572 rem
1575 rem ** move north **
1578 if gh(l,g)<>1 then goto 1600
...
1590 gh(l,g)=1:gh(y,g)=gh(y,g)-yoff:goto 1770
1595 rem ** move south **
1600 if gh(l,g)<>2 then goto 1620
...
1610 gh(l,g)=2:gh(y,g)=gh(y,g)+yoff:goto 1770
1615 rem ** move east **
1620 if gh(l,g)<>3 then goto 1640
...
1630 gh(l,g)=3:gh(x,g)=gh(x,g)+1:goto 1770
...
1645 gh(l,g)=4:gh(x,g)=gh(x,g)-1
1650 rem
1770 poke gh(y,g)+gh(x,g),65
